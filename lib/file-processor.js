...
function runUUIDProcess(scriptFileName) {
  ...
  // Check if we have specific target files
  if (config.targetFiles && config.targetFiles.length > 0) {
    console.log(`Processing ${config.targetFiles.length} target files...`);
    
    // Process each target file
    for (const targetPath of config.targetFiles) {
      const fullPath = path.join(process.cwd(), targetPath);
      
      if (!fs.existsSync(fullPath)) {
        console.log(`Warning: Target file not found: ${targetPath}`);
        continue;
      }
      
      if (path.basename(fullPath) === scriptFileName) {
        console.log(`Skipping self: ${fullPath}`);
        continue;
      }
      ...
      if (/\.js$/.test(fullPath)) {
        ...
        console.log(`Skipping non-JavaScript file: ${fullPath}`);
      }
    }
  } else {
    // Process all files in the directory (existing behavior)
    console.log('No specific target files configured. Processing all JS files...');
    const dirResults = processDirectory(process.cwd(), scriptFileName);
    modifiedFiles.push(...dirResults);
  }
